...
func (d *AstarteFDOIngressCustomDefaulter) Default(_ context.Context, obj runtime.Object) error {
	...
	if !ok {
		return fmt.Errorf("expected an AstarteFDOIngress resource")
	}

	fdolog.Info("Defaulting for AstarteFDOIngress", "name", fdoIngress.GetName())

	if fdoIngress.Spec.IngressClass == "" {
		fdoIngress.Spec.IngressClass = "haproxy"
	}
...
func validateAstarteFDOIngress(fdoIngress *ingressv2alpha1.AstarteFDOIngress) error {
	...
	astarte, astarteFoundErr := validateFDOReferencedAstarte(fdoIngress)
	if astarteFoundErr != nil {
		allErrors = append(allErrors, astarteFoundErr)
	}

	if astarteFoundErr == nil {
		if err := validateFDOEnabled(astarte); err != nil {
			allErrors = append(allErrors, err)
		}
		if err := validateFDOAPIHost(astarte); err != nil {
			allErrors = append(allErrors, err)
		}
	}
...
func validateFDOEnabled(astarte *apiv2alpha1.Astarte) *field.Error {
	if astarte.Spec.FDO != nil && astarte.Spec.FDO.Enable {
		return nil
	}

	return field.Invalid(
		field.NewPath("spec").Child("astarte"),
		astarte.Name,
		"FDO must be enabled in the referenced Astarte resource",
	)
}

func validateFDOAPIHost(astarte *apiv2alpha1.Astarte) *field.Error {
	if astarte.Spec.API.Host != "" {
		return nil
	}
	...
	return field.Invalid(
		field.NewPath("spec").Child("astarte"),
		astarte.Name,
		"the referenced Astarte resource must define spec.api.host",
	)
}
